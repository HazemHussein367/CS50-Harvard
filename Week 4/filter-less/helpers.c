...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        for (int column = 0; column < width; column++)
        {
            ...
            int avgGray = round((image[row][column].rgbtBlue + image[row][column].rgbtGreen +
...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        for (int column = 0; column < width; column++)
        {
            ...
            int sepiaRed =
                round(.393 * image[row][column].rgbtRed + .769 * image[row][column].rgbtGreen +
                      .189 * image[row][column].rgbtBlue);
            int sepiaGreen =
                round(.349 * image[row][column].rgbtRed + .686 * image[row][column].rgbtGreen +
                      .168 * image[row][column].rgbtBlue);
            int sepiaBlue =
                round(.272 * image[row][column].rgbtRed + .534 * image[row][column].rgbtGreen +
